// app/pages/movie/category/[slug].vue
...
const category = computed(() => tmdb.getMovieCategory(slug.value))
...
const { data, status } = await tmdb.fetchMovieCategory(slug, page)

useHead({
	title: () => `${meta.value.siteName} - ${category.value?.title ?? 'Movies'}`,
})
